// CCrea un'array di oggetti di tipo Book
const books = [
    {
        "id": 1,
...
// Crea un ogetto con i nomi dei generi in inglese
const iconsByGenres = {
    adventure: 'adventure',
    sea: 'sea',
    novel: 'book',
...
// Crea un ogetto con i nomi dei generi  in italiano
const genresLabels = {
    adventure: 'Avventura',
    sea: 'Mare',
    novel: 'Romanzo',
...
//  fai partire la funzione displayBooks
displayBooks(books);
//
function displayBooks(books) {
    ...
    books.forEach((book) => {
        ...
        const tags = book.genres.map((genre) => {
            // console.log(genre)
            return `<div class="book-tag">${genresLabels[genre]}</div>`;
        }).join('');
        // console.log(tags);
        ...
        booksContainer.innerHTML += `
            <div class="book">
                ...
                <p class="book-reading-time">Tempo di lettura: ${Math.round(book.pages / 30)}</p>
            </div>
        `;
    })
}
